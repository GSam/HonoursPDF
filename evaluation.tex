...
Hopefully, this work provides some form of contribution to the refactoring community.
...
\item User study - Why this is inappropriate? Could bring in users of Rust, but the language is not that popular still with only (XXX users). Finding the adequate amount of experience and users [target demographic] would likely be limited to those working heavily with the language, possibly working on the compiler. There would also be nothing to compare to, besides perhaps manual refactoring. The study would have to properly address availability and bias of people who know the language and its workings well. A good deal of time was spent just determining what could even be done as part of this tool and trying to juggle an user evaluation would have detracted from the goal of exploring the actual problems with a specifically Rust, refactoring tool. 
