...
% The assumption is that the inline will still have some kind of meaning. For the case of lifetimes, this is not always the case. In Figure X we can see the inlining of a vector. The problem with the resulting code is that when calling {\verb|iter()|} on the vector, vector must be disposed of. This is a violation of lifetimes. Besides running through all analysis again, it is not immediately clear how this case should be handled or whether or not it can be resolved in a much simpler way. 
